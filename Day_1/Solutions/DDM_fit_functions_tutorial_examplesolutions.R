...
Cost_ddm <- function(obs_RT, obs_acc, pred_RT, pred_acc, plotting){
  ...
  # highest prortion of trials for both?
  obs_prop_cor
  pred_prop_cor
...
Iterate_fit_4params <- function(params, Observations, CC, ntrials = 1000){
  
  names(params) <- c('v', 'a', 'ter', 'z')
  
  sim_data <-DDM_4params(v = params['v'],a = params['a'],ter=params['ter'],z=params['z'],CC, ntrials = ntrials)
